...
Важлива поведінка: endpoint перевіряє email, нормалізує URL, перевіряє наявні active/pending subscriptions і recipient confirmation-email throttle до OLX fetch. `Listing` і `Subscription` створюються тільки після того, як запит дозволений і ціну успішно отримано. Якщо оголошення не знайдено, запит throttled або ціна не витягнута, нові записи не створюються.
...
- `429` - confirmation email нещодавно вже надсилали на цю email-адресу.
- `400` - invalid JSON.
- `404` - OLX оголошення не знайдено.
- `422` - invalid email, invalid URL або ціну не вдалося витягнути.
- `502` - OLX/network fetch error або confirmation email transport failure.
...
Якщо confirmation email throttled, JSON response має status `confirmation_throttled`.
...
`consecutive_not_found_count` відстежує підтверджену недоступність: HTTP `404` і `410` мапляться в `not_found`. `consecutive_fetch_error_count` відстежує тимчасові збої, зокрема HTTP `5xx`, timeout і parsing failures, які мапляться в `parse_error`.
...
Email навмисно зберігається прямо в `subscriptions`. Для цього завдання окрема таблиця users/subscribers не потрібна: підписка сама і є зв'язком "цей email підписаний на це оголошення". `last_email_sent_at` використовується для простого throttling-а confirmation emails для тієї самої email-підписки.
...
11. Unavailable notification надсилається тільки після повторних підтверджених `not_found`, таких як HTTP `404` або `410`, не після timeout, HTTP `5xx` або parser error.
12. Email confirmation є subscription-scoped.
13. Повторний confirmation email для тієї самої email-адреси не надсилається частіше, ніж дозволяє `EMAIL_RATE_LIMIT_SECONDS`.
...
9. Для pending duplicate token оновлюється, і confirmation email надсилається повторно, якщо не спрацював email rate limit для адреси отримувача.
...
Логи пишуться через Monolog у standard container output. Дивитися їх можна так:
...
App логує subscription creation failures, notification failures, safe exception metadata, OLX HTTP status/latency і extractor source used. Worker логує listing ids/URLs, які перевіряються, status transitions, not-found/fetch-error counters і notification send failures.

Logging safety rules: не логувати `MAILER_DSN`, SMTP passwords/API keys, confirmation tokens, повний OLX HTML response або email bodies.
...
Тексти листів є plain text, без HTML templates. Мова листів визначається `LOCALE`: підтримуються `ua` та `en`, а невідоме значення fallback-иться до `ua`. Локалізація використовує Symfony Translation, а тексти зберігаються в `translations/emails.ua.yaml` і `translations/emails.en.yaml`. Назва сайту в листах підставляється з `PROJECT_NAME`.

Розділення відповідальностей:

- `EmailFactory` створює Symfony `Email`, встановлює `from`, `to`, `subject` і body.
- `EmailTemplateRendererInterface` описує рендеринг confirmation, price changed і listing unavailable листів.
- `SymfonyEmailTemplateRenderer` вибирає мову та отримує subject/body із Symfony Translation domain `emails`.
...
`EMAIL_RATE_LIMIT_SECONDS` задає мінімальну кількість секунд між confirmation emails для тієї самої email-адреси. Якщо повторний запит приходить раніше, сервіс не надсилає лист і не робить OLX fetch. Наявна pending subscription не міняє token і повертає JSON status `confirmation_throttled` з HTTP `429`; якщо немає subscription для повторного використання, нова pending subscription не створюється.

Якщо Symfony Mailer не може надіслати confirmation email, API повертає HTTP `502` з `{"status":"error","message":"Unable to send confirmation email."}`. Raw SMTP/provider error логуються для operators тільки в redacted form і не потрапляють в API response.
...
Лист надсилається тільки active subscribers і тільки після `OLX_UNAVAILABLE_NOTIFICATION_THRESHOLD` послідовних `not_found`. HTTP `404` і `410` рахуються як `not_found`. Timeout, 5xx, parser error або `no_price` не запускають unavailable notification. Повідомлення не залежить від `consecutive_fetch_error_count` і надсилається лише один раз, доки оголошення знову не стане доступним.

Після отриманого листа про зміну ціни тестове оголошення було деактивовано. Mailpit підтверджує, що сервіс пізніше надіслав лист про недоступність оголошення:
...
Фінальний порядок extraction:

1. `window.__PRERENDERED_STATE__`
2. JSON-LD
3. HTML fallback

`window.__PRERENDERED_STATE__` є primary source, бо OLX уже додає structured listing data в server-rendered HTML. JSON-LD і visible HTML залишаються fallback для стійкості.

HTTP `404` і `410` мапляться в `not_found`. Відсутність ціни на сторінці - в `no_price`. Інші помилки, зокрема HTTP `5xx`, timeout і parsing failures, мапляться у `parse_error`.
...
Після чистого clone або розпакування архіву Git усередині Docker-контейнера може відмовитися працювати з `/app`:

```text
fatal: detected dubious ownership in repository at '/app'
```

Це захист Git від роботи з репозиторієм, власник якого не збігається з користувачем процесу. У цьому проєкті `/app` є bind mount з хоста, тому таке повідомлення може з'явитися під час `composer qa`, `composer cs-check` або інших команд, які опосередковано звертаються до Git.

Важливо: команда без Docker-префікса налаштовує Git на хості, а не в PHP-контейнері. Тому правильна команда така:

```bash
docker compose exec app git config --global --add safe.directory /app
```

Після цього можна запускати перевірки:
